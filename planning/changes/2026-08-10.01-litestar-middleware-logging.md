...
Litestar 2.24 matches `exclude` against both the request path and the route
handler's paths, so `^/doc/static(?:/|$)` also matches the static router's
`/doc/static/{file_path:path}` and no Litestar-3 migration `DeprecationWarning`
fires.
