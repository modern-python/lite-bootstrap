...
- **A second bootstrapper sharing an application now fails loudly instead of corrupting it.**
  ...
  OPTIONS`; FastAPI did not fail at all — the app's route count silently grew from 6 to 8, a
  shadowed duplicate of the health-check and metrics routes. `bootstrap()` on the losing
  bootstrapper now raises `ConfigurationError` naming itself. If your code relied on the FastAPI
  case appearing to "work", it will now raise.
